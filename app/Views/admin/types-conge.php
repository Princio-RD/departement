...
<div class="app-wrap">
  <aside class="sidebar">
    ...
    <ul class="sidebar-nav" style="margin-top:1rem">
      ...
      <li><a href="<?= base_url('admin/types-conge') ?>" class="active"><i class="bi bi-tags"></i> Types de congé</a></li>
    </ul>
    ...
    <div class="content">
      <div class="data-card">
        <div class="data-card-head">
          <h3>Configuration des types</h3>
        </div>
        <table class="tbl">
          <thead>
            <tr><th>Libellé</th><th>Code</th><th>Jours annuels</th><th>Actions</th></tr>
          </thead>
          <tbody>
            <?php if (!empty($types)): ?>
              <?php foreach ($types as $type): ?>
              <tr>
                <td><strong><?= esc($type['libelle'] ?? '') ?></strong></td>
                <td><code><?= esc($type['code'] ?? '') ?></code></td>
                <td><?= (int)($type['jours_annuels'] ?? 0) ?> j</td>
                <td>
              ...
              <?php endforeach; ?>
            <?php else: ?>
              <tr><td colspan="4" style="text-align:center;padding:1rem;color:var(--muted)">Aucun type configuré</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
