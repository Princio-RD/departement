  ...
  <div class="main">
    ...
    <div class="content">
      ...
      <div class="form-section" id="ajout-employe">
        ...
        <form action="<?= base_url('admin/create-employe') ?>" method="post">
        ...
        <table class="tbl">
          ...
          <tbody>
            <?php if (!empty($employes)): ?>
              ...
              <tr>
                ...
                <td>
                  <div class="action-btns">
                    <?php if ($emp['actif']): ?>
                    <button class="btn-sm btn-edit" onclick="editEmploye(<?= (int)($emp['id'] ?? 0) ?>)"><i class="bi bi-pencil"></i> Éditer</button>
                    <button class="btn-sm btn-del" onclick="toggleEmploye(<?= (int)($emp['id'] ?? 0) ?>)"><i class="bi bi-slash-circle"></i></button>
                    <?php else: ?>
                    <button class="btn-sm btn-view" onclick="toggleEmploye(<?= (int)($emp['id'] ?? 0) ?>)"><i class="bi bi-arrow-counterclockwise"></i> Réactiver</button>
                    <?php endif; ?>
                  </div>
                </td>
              </tr>
              <?php endforeach; ?>
            <?php else: ?>
              <tr><td colspan="7" style="text-align:center;padding:1rem;color:var(--muted)">Aucun employé trouvé</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
...
function toggleEmploye(id) {
  if (confirm('Êtes-vous sûr de vouloir modifier le statut de cet employé ?')) {
    window.location.href = '<?= base_url('admin/toggle-employe') ?>/' + id;
  }
}

function editEmploye(id) {
  // TODO: implémenter l'édition (modal ou page dédiée)
  alert('Édition de l\'employé ' + id + ' - À implémenter');
}
</script>
</body>
</html>
