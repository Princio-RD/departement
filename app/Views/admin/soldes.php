...
<title>TechMada RH — Soldes annuels</title>
...
<div class="app-wrap">
  <aside class="sidebar">
    ...
    <ul class="sidebar-nav" style="margin-top:1rem">
      ...
      <li><a href="<?= base_url('admin/validation-rh') ?>"><i class="bi bi-inbox"></i> Toutes les demandes</a></li>
      ...
      <li><a href="<?= base_url('admin/soldes') ?>" class="active"><i class="bi bi-sliders"></i> Soldes annuels</a></li>
    </ul>
  ...
  <div class="main">
    <div class="topbar">
      <div>
        ...
        <div class="topbar-breadcrumb"><a href="<?= base_url('admin/dashboard') ?>">Admin</a> <i class="bi bi-chevron-right" style="font-size:.6rem"></i> Soldes</div>
      </div>
    </div>
    ...
    <div class="content">
      <div class="data-card">
        <div class="data-card-head">
          <h3>Soldes pour <?= $annee ?></h3>
        </div>
        <table class="tbl">
          <thead>
            <tr><th>Employé</th><th>Type</th><th>Attribué</th><th>Pris</th><th>Restant</th></tr>
          </thead>
          <tbody>
            <?php if (!empty($soldes)): ?>
              <?php foreach ($soldes as $solde): ?>
              <tr>
                <td><?= esc($solde['employe_nom'] ?? '') ?></td>
                <td><?= esc($solde['type_libelle'] ?? '') ?></td>
                <td><?= number_format($solde['jours_attribues'] ?? 0, 1) ?> j</td>
                <td><?= number_format($solde['jours_pris'] ?? 0, 1) ?> j</td>
                <td><strong><?= number_format(($solde['jours_attribues'] ?? 0) - ($solde['jours_pris'] ?? 0), 1) ?> j</strong></td>
              </tr>
              <?php endforeach; ?>
            <?php else: ?>
              <tr><td colspan="5" style="text-align:center;padding:1rem;color:var(--muted)">Aucun solde pour cette année</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
